...
class SFMovieQueryProcessor:
    ...
    def load_table_as_dataframe(self):
        """Load the entire database table into a pandas DataFrame.

        Reads data from SQLite and returns either a pandas DataFrame or,
        if geospatial columns exist, a GeoPandas GeoDataFrame.

        Returns:
            Union[pd.DataFrame, gpd.GeoDataFrame]: The loaded data with optional
            geospatial conversion if 'Lat'/'Lon' columns are present.
        """
        conn, _ = self._get_db_connection()
        # Get all data from the sf_film_data table
        df = pd.read_sql_query("SELECT * FROM sf_film_data", conn)
        conn.close()

        # If there are geospatial columns, convert to GeoDataFrame
    ...
    def generate_pandas_code(self, user_query, complexity):
        """Generate pandas/geopandas code based on the user query and complexity."""

        # Different prompts for different complexity levels
        if complexity == "PYTHON_PANDAS":
            prompt = self.pandas_analysis_prompt_maker(user_query)
        elif complexity == "PYTHON_VISUALIZATION":
            prompt = self.pandas_visualization_prompt_maker(user_query)
        else:
            return "Error: Unexpected complexity level for pandas processing"

        # Call Gemini to generate the pandas code
        response = call_generative_api(prompt, user_query)

        if not response:
            return "Failed to generate pandas code"

        # Extract the code from the response
        try:
            pandas_code = extract_code_blocks(response.text)
            return pandas_code[0] if pandas_code else "No valid code generated"
        except Exception as e:
            print(f"Error extracting pandas code: {e}")
            return f"Error processing pandas code: {str(e)}"
    ...
    def analyze(self):
        ...
        if complexity in ("SIMPLE_SQLITE", "COMPLEX_SQLITE"):
            ...
            # Only load the DataFrame when we need pandas/geopandas functionality
            self.df = self.load_table_as_dataframe()
            pandas_code = self.generate_pandas_code(cleaned_query, complexity)
            return pandas_code
